...
def solution(progresses, speeds):
    ...
    for i in range(len(progresses)):
        new_progresses.append(math.ceil((100 - progresses[i])/speeds[i]))

    result = []
    cnt = 1
    before_progress = new_progresses[0]

    for j in range(1,len(new_progresses)):
        if before_progress >= new_progresses[j]:
            cnt += 1
        else:
            result.append(cnt)
            before_progress = new_progresses[j]
            cnt = 1
    # 마지막 처리 삽입
    result.append(cnt)
    return result
